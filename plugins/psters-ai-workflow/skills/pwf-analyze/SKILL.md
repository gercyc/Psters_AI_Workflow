...
description: >
  ...
  Use when: user wants to validate a plan before implementation or check consistency across artifacts.
argument-hint: "[plan path in docs/plans/]"
---

# Cross-Artifact Analysis (Read-Only)

Use this skill to assess consistency and coverage before implementation.
...
1. **Duplication:** repeated requirements or acceptance criteria with conflicting phrasing.
2. **Ambiguity:** vague terms without measurable criteria.
3. **Underspecification:** requirements lacking concrete success criteria.
4. **Coverage:** requirement without task, task without requirement, NFRs not reflected in tasks.
5. **Inconsistency:** terminology drift, entity/API mismatch between sections.
6. **Rule/pattern alignment:** contradictions with critical project patterns.
...
Severity: `CRITICAL` (blocks safe implementation), `HIGH` (likely rework), `MEDIUM` (quality), `LOW` (polish).

At the end, ask: "Would you like concrete remediation edits for the top issues?"
